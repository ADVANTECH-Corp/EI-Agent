...
static BOOL LaunchPipeInst(PIPEINSTLIST pipeInstList)
{
   ...
   if(pipeInstList == NULL) return FALSE;
   {
      ...
      while(pCurPipeInstNode)
      {
         ...
         pCurPipeInstNode->pipeInst.pipeListenThreadHandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)PipeInstListenThreadStart, (LPVOID)&(pCurPipeInstNode->pipeInst), 0, NULL);
         ...
         pCurPipeInstNode->pipeInst.pipeRecvThreadHandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)PipeInstRecvThreadStart, (LPVOID)&(pCurPipeInstNode->pipeInst), 0, NULL);
...
WISEPLATFORM_API int NamedPipeServerInit(char * pName, DWORD instCnt)
{
   BOOL bRet = FALSE;
   DWORD i = 0;
...
WISEPLATFORM_API int IsChannelConnect(DWORD commID)
{
   BOOL bRet = FALSE;
   PPIPEINSTNODE findNode = NULL;
   if(PipeInstList == NULL) return bRet;
...
WISEPLATFORM_API int NamedPipeServerRegRecvCB(DWORD commID, PPIPERECVCB pipeRecvCB)
{
   BOOL bRet = FALSE;
...
WISEPLATFORM_API int NamedPipeServerSend(DWORD commID, char * sendData, DWORD sendToLen)
{
   return NamedPipeServerGNSend(USER_MSG, commID, sendData, sendToLen);
}
