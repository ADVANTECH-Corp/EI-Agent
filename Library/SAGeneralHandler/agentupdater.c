				...
				case FTP_TRANSFERRING:
					{
						...
						if(pUpdateParam->pSendCb)
						{
							...
							sprintf(downloadDetial,"Downloading,Download Cursize:%udKB, Persent:%ud%%, Speed: %4.2fKB/S\n", dCurSizeKB, dPercent, dSpeed);
				...
				case FTP_FINISHED:
					{
						...
						{
							char downloadDetial[128] = {0};
							sprintf(downloadDetial,"Download Finished,Download Cursize:%udKB, Persent:%ud%%, Speed: %4.2fKB/S\n", dCurSizeKB, dPercent, dSpeed);
							pUpdateParam->pSendCb(glb_update_cagent_rep, downloadDetial, strlen(downloadDetial)+1, NULL, NULL);
						}
						isBreak = true;
						break;
