...
bool message_create(struct message* msg, const char* topic, const void* payload, const int payloadlen, void* userdata)
{
	if(!msg)
		return false;
...
void* threat_message_queue(void* args)
{
	...
	while(precvContex->isThreadRunning)
	{
		...
		{
			g_on_message_func(msg->topic, msg->payload, msg->pauloadlen, msg->userdata);
		}
		message_free(msg);
		msg = NULL;
	}
...
bool msgqueue_push(const char* topic, const void* payload, const int payloadlen, void* userdata)
{
	if(g_recvthreadctx.msgqueue)
	{
		struct message *newmsg = malloc(sizeof(struct message));
		message_create(newmsg, topic, payload, payloadlen, userdata);
...
void msgqueue_on_recv(const char* topic, const void *pkt, const long pktlength, void* userdata)
{
	if(g_recvthreadctx.msgqueue)
	{
		struct message *newmsg = malloc(sizeof(struct message));
		message_create(newmsg, topic, pkt, pktlength, userdata);
		if(!queue_put(g_recvthreadctx.msgqueue, newmsg))
		{
			message_free(newmsg);
			newmsg = NULL;
