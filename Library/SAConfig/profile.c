...
bool SACONFIG_API profile_save(char const * configFile, susiaccess_agent_profile_body_t const * const profile)
{
	...
	if(!xml_SetItemValue(doc,DEVICE_NAME_KEY, profile->hostname))
		goto SAVE_EXIT;

	if(!xml_SetItemValue(doc,SW_VERSION_KEY, profile->version))
		goto SAVE_EXIT;
	...
	if(!xml_SetItemValue(doc, DEV_MANUFACTURE_KEY, profile->manufacture))
		goto SAVE_EXIT;

	if(!xml_SetItemValue(doc, DEV_LAL_KEY, profile->lal))
		goto SAVE_EXIT;

	if(!xml_SetItemValue(doc, DEV_WORKDIR_KEY, profile->workdir))
		goto SAVE_EXIT;

	if(!xml_SetItemValue(doc,USER_NAME_KEY, profile->account))
		goto SAVE_EXIT;

	if(!xml_SetItemValue(doc,USER_PASSWORD_KEY, profile->passwd))
		goto SAVE_EXIT;
