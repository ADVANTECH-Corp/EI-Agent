...
SALOG_EXPORT void SALOG_CALL PrintAndWriteLog(int level, const char *file, const char *func, const char *line, const char* levels, const char *fmt, ...)
{
	...
	if (AdvLog_AllNone("default", level)) {
		...
		AdvLog_VPrintAndWrite("default", level, COLOR_NONE, NULL, NULL, file, func, line,levels, fmt, ap);
	} else if(AdvLog_Dynamic_Is_On("default", level) && !AdvLog_Static_Is_On("default", level) && !AdvLog_Dynamic_Is_Hiden("default", level)) {
		AdvLog_VPrint("default", level, COLOR_NONE,file, func, line,levels, fmt, ap);
	} else if(AdvLog_Static_Is_On("default", level)) {
		AdvLog_VWrite("default", level, NULL,NULL,file, func, line,levels, fmt, ap);
	}
	va_end(ap);	
}
