...
bool hlloader_is_exist(const char* path)
{
...
bool hlloader_load(const char* path, SALoader_Interface * SALoader)
{
	bool bRet = false;
...
void hlloader_handler_release(SALoader_Interface* pSALoader, Handler_List_t *pHandlerList)
{
	...
	else if(pSALoader->Loader_ReleaseAllHandler_API)
		pSALoader->Loader_ReleaseAllHandler_API(pHandlerList);
}
